...
// Modificar a estrutura UpdaterState para rastrear tanto verificações quanto instalações
struct UpdaterState {
  checking: AtomicBool,
  installing: AtomicBool,  // Novo campo para rastrear instalações em andamento
}
...
// Modificar a função install_update_from_backend para usar o novo campo
#[tauri::command]
async fn install_update_from_backend(app_handle: AppHandle, updater_state: State<'_, UpdaterState>) -> Result<(), String> {
  println!("Instalando atualização a partir do backend...");
  
  // Verificar se já existe uma instalação em andamento
  if !updater_state.installing.compare_exchange(false, true, Ordering::SeqCst, Ordering::SeqCst).is_ok() {
    return Err("Já existe uma atualização em andamento".to_string());
  }
  
  // Garantir que o estado seja restaurado quando a função terminar
  let result = async {
  ...
  // Restaurar o estado para permitir futuras instalações
  updater_state.installing.store(false, Ordering::SeqCst);
